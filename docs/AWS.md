...
3. You get two values: an access key ID and a secret access key. Treat
   ...
   over a channel you trust - not email, not a ticket.
...
## Questions this guide has been asked
...
A dedicated account keeps the blast radius smaller; an existing one
keeps billing consolidated. Both work.
